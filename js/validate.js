...
pristine.addValidator(hashtagsInput, (value) => value === '' || parseHashtagsInput(value).every((item) => hashtagEndingRegExp.test(item)),
  'Хэштеги разделяются пробелом');

pristine.addValidator(hashtagsInput, (value) => isArrayUnique(parseHashtagsInput(value)),
  'Хэштеги не должны повторяться');

pristine.addValidator(hashtagsInput, (value) => value === '' || parseHashtagsInput(value).every((item) => hashtagRegExp.test(item)),
  'Хэштег начинается с # и состоит только из букв и цифр');

pristine.addValidator(hashtagsInput, (value) => value === '' || parseHashtagsInput(value).every((item) => item.length >= HashtagLength.MIN && item.length <= HashtagLength.MAX),
